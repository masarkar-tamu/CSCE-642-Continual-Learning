...
class FixedAgent(Agent):
    ...
    def act(self, _=None):
        ...
        # Implements n-step removed from fixed policy experiments
        if cfg.step_off != 0 and (self.acts % len(self.plan) == 0 or self.acts == 1):
            self.cycle_count = 0
            self.interference_indices = sorted(
                random.sample(list(range(sum(self.plan))), cfg.step_off)
            )

        if (
            len(self.interference_indices) != 0
            and self.cycle_count == self.interference_indices[0]
        ):
