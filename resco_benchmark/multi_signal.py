...
class MultiSignal(gym.Env):
    ...
    def __init__(self, state_fn, reward_fn):
        ...
        #     self.action_mask = getattr(action_sets, cfg.action_set)(self.obs_act, self.signals)
        #     for ts in self.obs_act:
        #         self.obs_act[ts] = (self.obs_act[ts][0], self.action_mask.num_acts)
        #
        # if cfg.auto_scale:
        #     self.state_fn, self.reward_fn = self.collect_scale(observations, state_wrapped, reward_fn)
        #
    ...
    def step(self, act):
        # Fixed signal shadow controllers only used for specific experiments
        # Will not effect anything unless cfg.controlled_signals is not null
